...
const neatCSV= require("neat-csv")
 ...
 describe('My cypress login token Test Suite', function() {
     ...
     it("end to end scenario with CSV",function(){
        let numberOfItem =1
        let productName 
        cy.visit("https://rahulshettyacademy.com/client",
        {
    ...
    it("end to end scenario with excel", function() {
        let numberOfItem = 1;
        let productName;
        cy.visit("https://rahulshettyacademy.com/client", {
            onBeforeLoad: function(window) {
        ...
        // Handling the file read and conversion correctly
        const downloadPath = "cypress/downloads/order-invoice_anshika.xlsx";
        cy.readFile(downloadPath).then(excelData => {
            cy.task('excelconvertToJson', downloadPath).then(function(result) {
                console.log(result);
                cy.log(result)
